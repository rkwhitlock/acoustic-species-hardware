 ...
 ** JOYSTICK POLLING**
Use the joystick button (configured in POLLING mode) to move a pointer inside a rectangle 
(up/down/right/left) and change the pointer color(select).

 ** JOYSTICK EXTI**
Use the joystick button (configured in EXTI mode) to move a pointer inside a rectangle 
(up/down/right/left) and change the pointer color(select).
 ...
 ** Camera **
This example shows how to use the Camera with different resolutions and effect.
 ...
 ** AUDIO RECORD **
This example shows how to record an audio stream from microphnes mounted on STM32H747I-DISCO
and play it using the DMA circular mode and how to handle the buffer update.
Plug a headphone to hear the sound.  

 ** ANALOG AUDIO RECORD **
This example shows how to record an audio stream from the analog microphne connected through the Jack in blue of the STM32H747I-DISCO
and play it using the DMA circular mode and how to handle the buffer update.
Plug on CN10 a micro to record the sound.
Plug on CN11 a headphones to listen to the recorded audio.
