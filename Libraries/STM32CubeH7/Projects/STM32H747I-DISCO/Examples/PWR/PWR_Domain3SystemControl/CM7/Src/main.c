...
int main(void)
{
  ...
  /* Configure the MPU attributes as Non Cacheable for D3 SRAM */
  MPU_Config();
  ...
  */
  ...
  BDMA_Handle0.Init.Request              = BDMA_REQUEST_GENERATOR0;
  ...
  /* Deactivate all I2C4 interrupts and events for CM7 */
  ...
  /* Deactivate all I2C4 interrupts and events for CM4 */
  HAL_EXTI_D2_EventInputConfig(I2C4_WAKEUP_EXTI_LINE, I2C4_WAKEUP_EXTI_MODE, DISABLE);
  HAL_EXTI_D2_EventInputConfig(I2C4_EVENT_INTERRUPT_EXTI_LINE, I2C4_WAKEUP_EXTI_MODE, DISABLE);
  HAL_EXTI_D2_EventInputConfig(I2C4_ERROR_INTERRUPT_EXTI_LINE, I2C4_WAKEUP_EXTI_MODE, DISABLE);
  
  /* Enable BDMA Channel 7 interrupt for CM7 */
  HAL_EXTI_D1_EventInputConfig(EXTI_LINE73, I2C4_WAKEUP_EXTI_MODE, ENABLE);
  
  /* Disable BDMA Channel 7 interrupt for CM4 */
  HAL_EXTI_D2_EventInputConfig(EXTI_LINE73, I2C4_WAKEUP_EXTI_MODE, DISABLE);

  /* Disable BDMA Channel 0 interrupt for CM7 */
  HAL_EXTI_D1_EventInputConfig(EXTI_LINE66, I2C4_WAKEUP_EXTI_MODE, DISABLE);
  
   /* Disable BDMA Channel 0 interrupt for CM4 */
  HAL_EXTI_D2_EventInputConfig(EXTI_LINE66, I2C4_WAKEUP_EXTI_MODE, DISABLE);
  ...
  while(1)
  {
    ...
    if(bufferIndex >= BUFFER_SIZE)
    {
      ...
      /* -11- Toggle LED1 each 500 ms if the transmission and reception are passed */
      HAL_Delay(100);
      BSP_LED_Toggle(LED1);
    }
  }
...
static void Flush_Buffer(uint8_t *buffer, uint32_t size)
{
  ...
  /* Make CPU cache clean operation to ensure that data are written in D3SRAM */
  CPU_CACHE_Clean();
}
...
static void MPU_Config(void)
{
  ...
  /* Configure the MPU attributes as as Normal Non Cacheable for D3 SRAM */
  MPU_InitStruct.Enable = MPU_REGION_ENABLE;
