/**
  ******************************************************************************
  * @file    PWR/PWR_Hold_Mechanism/CM7/Src/main.c
...
int main(void)
{
  ...
  */
  ...
  /* Set RTC wakeup timer as wakeup source for CM7 */
  HAL_EXTI_D1_EventInputConfig(EXTI_LINE19, EXTI_MODE_IT, ENABLE);
  
  /* Set RTC wakeup timer as wakeup source for CM4 */
  ...
  while(1)
  {
    /* Turn On LED1 to indicate that CM7 is wokenup from STOP mode */
    BSP_LED_On(LED1);
...
#ifdef CM4_HOLD_TEST    
    /* Release CM4 core (here CM4 will wakeup from STOP mode and LED2 will turn On) */
    ...
    {
#ifdef CM4_HOLD_TEST 
      /* Set CM7 core as a master and hold CM4 as slave core */
      HAL_PWREx_HoldCore(PWR_CORE_CPU2);
#endif  

      /* Turn Off LED1 and enter D1 domain to DStop mode */
      BSP_LED_Off(LED1);
