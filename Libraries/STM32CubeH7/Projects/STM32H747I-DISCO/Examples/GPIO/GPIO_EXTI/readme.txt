...
The GPIO PC13 (connected to the Wakeup button) is configured in EXTI mode for both CPUs by the CPU1 (Cortex-M7)
When rising edge is detected on EXTI15_10 by pressing an releasing Wakeup push-button, 
each CPU will fall in its corresponding EXTI interrupt routine where we toggle a dedicated LED. 
	    LED1 toggles once (To indicated an EXTI interrupt for CPU1)
            LED2 toggles once (To indicated an EXTI interrupt for CPU2)
