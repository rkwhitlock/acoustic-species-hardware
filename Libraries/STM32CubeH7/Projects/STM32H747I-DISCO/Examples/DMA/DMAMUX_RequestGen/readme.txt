...
buffer (SRC_Buffer_LED4_Toggle) to the GPIOI ODR register (in order to toggle LED4).
...
@note the Domain 3 BDMA has access to  Domain 3 SRAMs and peripherals only,  thus the source buffer (SRC_Buffer_LED4_Toggle)
...
As consequence the DMA will serve the request and write a new value to the GPIOI ODR register to toggle the LED4
without any CPU intervention.
As consequence LED4 will toggle every 2sec.
